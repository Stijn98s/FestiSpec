...
export default class FPlanningItem extends Vue {
        @Prop() availability!: Availability;
            ...
            set isAvailable (available: any) {
              this.availability.isAvailable = available == 'Beschikbaar'
              console.log(this.availability.isAvailable)
              this.saveUpdate()
            }
            ...
            private async saveUpdate () {
              let availability
              if (this.availability.id) {
                availability = await availabilityClient.edit(this.availability)
              } else {
                availability = await availabilityClient.create(this.availability)
              }
              if (availability) {
